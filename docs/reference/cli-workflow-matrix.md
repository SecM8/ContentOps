...
| `contentops report` | `report.yml` | weekly Mon 08:00 UTC + push to main (detection/report paths) + manual | Regenerates the detection inventory / portfolio report; push-to-main commits the regenerated `reports/` as versioned content (durable posture history). Per-detection telemetry is kept off the **public mirror** by the sync allowlist (only `reports/badge.json` mirrors), not gitignore. `tenant.yml` `reports.retentionDays` bounds the committed dated history (CLI override: `--retention-days`). See [`durable-reports.md`](../operations/durable-reports.md). |
