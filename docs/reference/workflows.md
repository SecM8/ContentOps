...
| `report.yml` | weekly cron + push to main + manual | Generate the detection inventory report and upload it as the `Detection inventory report` artifact (`latest.*` + dated snapshot + `unified.html`); push-to-main commits **only** `reports/badge.json` — the detailed report carries per-detection telemetry and is gitignored. | `contents: write` | ~2 min |
