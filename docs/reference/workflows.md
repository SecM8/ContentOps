...
| `report.yml` | weekly cron + push to main + manual | Generate the detection inventory report and upload it as the `Detection inventory report` artifact; push-to-main commits the regenerated `reports/` (versioned posture history — `latest.*` + dated snapshots + `unified.html`). Per-detection telemetry is kept off the **public mirror** by the sync allowlist (only `reports/badge.json` mirrors), not gitignore. `tenant.yml` `reports.retentionDays` caps the committed dated history (materialises `tenant.yml` from `TENANT_CONFIG_YAML` when present). See [`durable-reports.md`](../operations/durable-reports.md). | `contents: write` | ~2 min |
